...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  float comforatable_deaccel = 1.0;
  auto look_ahead_distance = pow(velocity_mag, 2) / 2.0 / comforatable_deaccel;
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    if (is_goal_in_junction) {
      ...
      goal.location.x += goal.location.x + std::cos(ang);
      goal.location.y += goa.location.y + std::sin(ang);
      ...
      goal.velocity.x = _speed_limit * std::cos(goa.rotation.yaw);
      goal.velocity.y = _speed_limit * std::sin(goa.rotation.yaw);
      goal.velocity.z = 0;
    }
